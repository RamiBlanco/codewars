...
def cakes(recipe, available):
    total_cakes = [0] 
    for my_ingredient, my_quantity in available.items():
        for ingredient, quantity in recipe.items():
            if (my_ingredient == ingredient):
                if quantity != 0:
                    if my_quantity/quantity < 1:
                        return 0
                    else:
                        total_cakes.append(int(my_quantity/quantity))
            else:
                if ingredient not in available:
                    return 0
    total_cakes.sort()
    if (len(total_cakes) > 1):
        return total_cakes[1]    
    return total_cakes[0]
...
recipe_3 = {"flour": 0, "sugar": 0, "eggs": 0}
available_3 = {"flour": 1200, "sugar": 1200, "eggs": 5, "milk": 200}

print(cakes(recipe, available))
print(cakes_2(recipe_3, available_3))